%Single Unit Analysis version 2
%
% DIFFERENCE = ignore first 1000 msecs when visual event occurs
%
...
window.stimulus = 5000; %number of milliseconds the stimulus is on
...
window.prestim = 2500; %number of milliseconds before stimulus to calculate baseline (also used to plot post stimulus spikes)
...
for i = 1:size(data,2)-1 %to repeat the following loop for the number of neurons recorded in this penetration
    ...
    for k = 1:length(eye_open)
        ...
        for j = 1:numberoftrials
            ...
            %length(in msecs)and then divided by 1000 to convert to
            %spikes/sec. (FR 1000 msecs later)
            firingrate.(name).(eye_open{k}).evoked.per_trial(j) = sum(data(timestamps.events.(eye_open{k})(j)+1000:timestamps.events.(eye_open{k})(j)...
                +window.firing_window,i))./((window.firing_window-1000)./1000);
...
for i = 1:size(data,2)-1
    ...
    for k = 1:length(eye_open)
        ...
        %finds the maximum firing rate during the stimulus (starting 1000
        %msecs after start of visual event)
        max_fr.(name).(eye_open{k}).firing_rate = max(spike_count.(name).(eye_open{k})(centers >= 1000 & centers <= window.stimulus));
        
        %finds the all the times this firing rate is reached
        max_fr.(name).(eye_open{k}).time_max =...
            centers(spike_count.(name).(eye_open{k}) == max(spike_count.(name).(eye_open{k})(centers >= 1000 & centers <= window.stimulus)));
